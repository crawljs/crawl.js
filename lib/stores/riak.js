...
function Riak(name, _options) {
  ...
  this.db = riak.getClient({ host: options.host, port: options.port, encodeUri: true});
...
Riak.prototype.end = function (chunk) {
  ...
  self.db.save('urls.' + conf.block + '.data', self.key, { headers: self.headers, body: self.buffer});
