...
Fetcher.init = function () {
  ...
  request.defaults(conf.fetcher.request);
...
Fetcher.prototype.onError = function () {
  var self = this;
  return function (err) {
    //make sure to abort the request so that `onFinish` is not called
    self.req.abort();
...
Fetcher.prototype.onFinish = function () {
  ...
  return function () {
    if (self.error) {
      throw new Error('Fatal! `onFinish` should not be called in case of an error! url to blame: ' + self.url);
    } else {
      //all good
...
Fetcher.prototype.get = function (url, cb) {
  ...
  //on error
  self.req.on('error', self.onError());
  self.req.on('end', self.onFinish());
