...
function Fetcher (name, queue) {

  this.name = name;

  //every fetcher has its own extractor & store

  this.store = store.create(name);
  this.store.queue = queue;

  this.extractor = extractor.create(name);
  this.extractor.queue = queue;
...
module.exports = function (count, queue) {

  fetchers = [];

  for (var i = 0; i < count; i++) {
    fetchers.push(new Fetcher('fetcher-' + i, queue));
  }
