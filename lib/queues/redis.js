...
function keyDone () {
  return 'group:' + conf.group + ':block:' + conf.block + ':done';
}

function keyNew (group, block) {
  return 'group:' + group + ':block:' + block + ':new';
}
...
Redis.prototype.peek = function (cb) {
  ...
  this.db.sdiff(keyNew(conf.group, conf.block), keyDone(), cb);
};

Redis.prototype.enqueue = function (entry, group, block) {

  var self = this
    , key = keyNew(group, block)
